...
const dbPath = process.env.DATABASE_FILE || path.join(process.cwd(), 'data', 'alerts.db');
...
-- Trigger to auto-update updated_at timestamp on rule changes
...
if (ADMIN_USERNAME && ADMIN_EMAIL) {
  try {
    ...
    if (!exists) {
      const info = db
        .prepare('INSERT INTO users (username, email, name, role) VALUES (?, ?, ?, ?)')
        .run(ADMIN_USERNAME, ADMIN_EMAIL, ADMIN_USERNAME, 'admin');
      console.log(`Seeded admin user id=${info.lastInsertRowid} (${ADMIN_USERNAME})`);
    } else {
