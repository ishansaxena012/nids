...
#include "packet_sniffer.h"
#include <pcap.h>            // must include pcap before using PCAP_ERRBUF_SIZE, etc.
#include <iostream>
#include <fstream>
#include <sstream>
#include <ctime>
...
PacketSniffer::PacketSniffer(int device_num) : handle(nullptr) {
    ...
    int idx = 1;
    while (dev && idx < device_num) { dev = dev->next; ++idx; }

    if (!dev) {
        cerr << "Error: Device number " << device_num << " not found." << endl;
        // print available devices for convenience
...
void PacketSniffer::process_packet_with_len(const u_char* packet_data, bpf_u_int32 packet_len) {
    if (!packet_data || packet_len < 14) return; // must have ethernet header
    ...
    if (proto == 1) {
        ...
        if (b0 == 10) src_is_private = true;
        else if (b0 == 192 && b1 == 168) src_is_private = true;
        else if (b0 == 172 && (b1 >= 16 && b1 <= 31)) src_is_private = true;
    ...
    // --- UDP handling (protocol 17) - ignored for now ---
    if (proto == 17) {
        return;
    }
...
if (proto == 6) {
    ...
    if (packet_len < tcp_off + 20) return;
    ...
    if (tcp_hdr_len < 20 || packet_len < tcp_off + tcp_hdr_len) return;
    ...
    // explicit important ports (if you want these alerted even if whitelisted remove them from SAFE_PORTS)
    if (dst_port == 22) {
    ...
    if (is_syn && !is_ack && !is_rst && !is_fin && !is_psh) {
        ...
        if (rec.syns == 0) {
            ...
            if (elapsed_ms <= 5000) { // 5 second window
                rec.syns++;
            } else {
        ...
        if (rec.syns > SYN_THRESHOLD) {
            ...
            rec.syns = 0; // reset after reporting
        }
        // do not emit an alert for each single SYN to reduce noise
        return;
    }

    // RST observed — treat as suspicious on non-whitelisted ports
    if (is_rst) {
